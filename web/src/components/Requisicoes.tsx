...
import { Plus, Search, Clock, CheckCircle, XCircle, AlertCircle, User, Loader2, Download } from 'lucide-react';
import { requisicoesAPI } from '../services/api';
import { useAuth } from '../context/AuthContext';
import toast from 'react-hot-toast';
import { exportRequisicoesToExcel, exportRequisicoesToPDF } from '../utils/exportUtils';
...
export function Requisicoes() {
  ...
  const handleCreate = async () => {
    if (!isAdmin) {
      toast.error('Apenas administradores podem criar novas requisições');
      return;
    }
          ...
          <button
            ...
            onClick={(e) => {
              ...
              try {
                ...
                exportRequisicoesToExcel(requisicoes);
            ...
            disabled={!requisicoes || requisicoes.length === 0}
            className="flex items-center gap-2 px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white hover:bg-white/10 transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          <button
            ...
            onClick={(e) => {
              ...
              try {
                ...
                exportRequisicoesToPDF(requisicoes);
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <span className="text-white/60 text-sm">{req.codigo}</span>
                      <div
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <div className="text-white/60 text-sm">{formatDate(req.data)}</div>
                    </div>
                  </div>
                  <div className="flex items-start gap-4">
                    <div className="text-right">
                      <p className="text-white/60 text-sm mb-1">Valor Estimado</p>
                      <p className="text-white text-xl">{formatCurrency(req.valor)}</p>
                    </div>
                    <div
